...
var (
	...
	processing = []ProcessingMessage{
		{TMESS_DEAUTH, nil},
		{TMESS_VERSION, process.ProcessVersion},
		{TMESS_AUTH, process.ProcessAuth},
		{TMESS_LOGIN, process.ProcessLogin},
		{TMESS_NOTIFICATION, process.ProcessNotification},
		{TMESS_REQUEST, process.ProcessConnect},
		{TMESS_CONNECT, nil},
		{TMESS_DISCONNECT, process.ProcessDisconnect},
		{TMESS_REG, process.ProcessReg},
		{TMESS_CONTACT, process.ProcessContact},
		{TMESS_CONTACTS, process.ProcessContacts}, //10
		{TMESS_LOGOUT, process.ProcessLogout},
		{TMESS_CONNECT_CONTACT, process.ProcessConnectContact},
		{TMESS_STATUSES, process.ProcessStatuses},
		{TMESS_STATUS, process.ProcessStatus},
		{TMESS_INFO_CONTACT, process.ProcessInfoContact},
		{TMESS_INFO_ANSWER, process.ProcessInfoAnswer},
		{TMESS_MANAGE, process.ProcessManage},
		{TMESS_PING, process.ProcessPing},
		{TMESS_CONTACT_REVERSE, process.ProcessContactReverse},
		{TMESS_SERVERS, process.ProcessServers}, //20
		{TMESS_STANDART_ALERT, nil}}

	processingAgent = []ProcessingAgent{
		{TMESS_AGENT_DEAUTH, nil},
		{TMESS_AGENT_AUTH, services.ProcessAgentAuth},
		{TMESS_AGENT_ADD_CODE, services.ProcessAgentAddCode},
		{TMESS_AGENT_DEL_CODE, services.ProcessAgentDelCode},
		{TMESS_AGENT_ADD_BYTES, services.ProcessAgentAddBytes},
		{TMESS_AGENT_NEW_CONN, services.ProcessAgentNewConn},

		18: {TMESS_AGENT_PING, services.ProcessAgentPing}}

	//функции для обработки web api
	processingWeb = []ProcessingWeb{
		{"defaultvnc", services.ProcessApiDefaultVnc},
		{"listvnc", services.ProcessApiListVnc},
		{"getlog", services.ProcessApiGetLog},
		{"clearlog", services.ProcessApiClearLog},
		{"profile_save", services.ProcessApiProfileSave},
		{"profile_get", services.ProcessApiProfileGet},
		{"save_options", services.ProcessApiSaveOptions},
		{"options_save", services.ProcessApiOptionsSave},
		{"reload", services.ProcessApiReload},
		{"reopen", services.ProcessApiReopen},
		{"options_get", services.ProcessApiOptionsGet},
		{"version", services.ProcessApiVersion}}
